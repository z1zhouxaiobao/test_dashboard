<template>
  <div class="page-card">
    <h2 class="page-title">{{ t('adminCases') }}</h2>
    <TableToolbar v-model="keyword" :placeholder="t('phCase')" @search="handleSearch" @refresh="loadData">
      <el-button type="danger" plain :disabled="!selected.length" :loading="deleting" @click="handleBatchDelete">{{ t('batchDelete') }}</el-button>
      <el-button type="primary" @click="openDialog()">{{ t('addCase') }}</el-button>
    </TableToolbar>
    ...
    <el-table v-loading="loading" :data="list" border align="center" header-align="center" row-key="id" @selection-change="onSelectionChange">
      ...
      <el-table-column prop="industry" :label="t('industry')" align="center" header-align="center" />
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? t('editCase') : t('addCase')" width="860px" top="5vh">
      <el-form :model="form" label-width="100px">
        <el-form-item :label="t('title')"><el-input v-model="form.title" /></el-form-item>
        <el-form-item :label="t('industry')"><el-input v-model="form.industry" /></el-form-item>
        ...
        <I18nCollapse :model="form" :fields="[{ base: 'title' }, { base: 'industry' }, { base: 'summary', type: 'textarea', rows: 2 }, { base: 'content', type: 'textarea', rows: 3 }]" />
      </el-form>
      <template #footer>
...
const form = reactive({
  id: null, title: '', titleTw: '', titleEn: '', industry: '', industryTw: '', industryEn: '',
  summary: '', summaryTw: '', summaryEn: '', coverUrl: '', content: '', contentTw: '', contentEn: ''
})

async function loadData() {
  loading.value = true
  try {
    const res = await caseApi.page({ page: page.value, size: size.value, keyword: keyword.value })
...
async function handleSave() {
  saving.value = true
  try {
    if (form.id) await caseApi.update(form.id, form)
